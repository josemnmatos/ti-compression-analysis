...
def benchmark(file):
    ...
    for run in range(NUMBER_OF_RUNS):
        ...
        # run commmand
        compress_LZMA2(file, "./benchmark_files/"+filename+".7z")
        while(True):
            end_time = time.time()
            if(exists(file)):
                break
    ...
    for run in range(NUMBER_OF_RUNS):
        ...
        start_time = time.time()
        # run commmand
        compress_Bzip2(file, "./benchmark_files/"+filename+".bz2")
        while(True):
            end_time = time.time()
            if(exists(file)):
                break
    ...
    for run in range(NUMBER_OF_RUNS):
        start_time = time.time()
        # run commmand
        compress_Deflate(file, "./benchmark_files/"+filename+".gz")
        while(True):
            end_time = time.time()
            if(exists(file)):
                break
...
def main():
    ...
    results = open("./benchmark_files/results.txt", 'w')
